...
<p>
<?php
echo "<span style='color:red'> The results have already been declared .</span>";
...
<?php }
  ...
  if(!empty($_POST["studiesclass"])) 
{
 $id= $_POST['studiesclass'];
 $dta=explode("$",$id);
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT StudentId,ClassId FROM midresult WHERE StudentId=:id1 and ClassId=:id ");
